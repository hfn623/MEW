...
//必须重定义钩子
__weak void mew_mc20_MallocErr_Hook(void){}
__weak void mew_mc20_DelayMS_Hook(uint32_t timespan){}
__weak void mew_mc20_SendBuff_Hook(uint8_t *buff, uint16_t len){}
//可选重定义钩子
__weak void *mew_mc20_Malloc_Hook(uint16_t size)
{
	return malloc(size);
}
__weak void mew_mc20_Free_Hook(void *p)
{
	free(p);
}
//事件钩子
__weak void mew_mc20_ResetStart_Hook(void){}
__weak void mew_mc20_Reset_Hook(void){}
__weak void mew_mc20_IdleHook(void){}

__weak void mew_mc20_GPRSConnDone_Hook(void){}
__weak void mew_mc20_GPRSConnErr_Hook(void){}

__weak void mew_mc20_SocketConnDone_Hook(uint8_t ch){}
__weak void mew_mc20_SocketConnErr_Hook(uint8_t ch, int8_t reason){}
__weak void mew_mc20_SocketDisconn_Hook(uint8_t ch, int8_t reason){}
__weak void mew_mc20_SocketHeartbeat_Hook(uint8_t ch)
{
	socketPush(ch, heartbeatPack.pBuff, heartbeatPack.Length); 
}
__weak void mew_mc20_SocketSendStart_Hook(uint8_t ch){}
__weak void mew_mc20_SocketSendDone_Hook(uint8_t ch){}
__weak void mew_mc20_SocketSendErr_Hook(uint8_t ch){}
__weak void mew_mc20_SocketRecvStart_Hook(uint8_t ch){}
__weak void mew_mc20_SocketRecvDone_Hook(uint8_t ch, uint8_t *buff, uint16_t len){}
__weak void mew_mc20_SocketRecvErr_Hook(uint8_t ch){}
__weak void mew_mc20_SocketRecvParseErr_Hook(uint8_t ch){}
...
{	
	...
	mew_mc20.WorkState = MC20_WS_RESETING;		
	mew_mc20.Socket_Schedule_NoOS = socket_Schedule_NoOS;
//	mew_mc20.HTTP_Schedule_NoOS = HTTP_Schedule_NoOS;
	mew_mc20.SocketSend = socketPush;	
	mew_mc20.IsReceiving = is_recv;

//	mew_mc20.HTTP_POST = httpPOST;
	
	mew_mc20.SocketEnable = socketEnable;
	mew_mc20.SocketEnableGet = socketEnableGet;
	mew_mc20.SocketDisable = socketDisable;
	...
	mew_mc20.HeartbeatInterval_Sec = 60;
	mew_mc20.ConnectionInterval_Sec = 10;
	mew_mc20.ReadInterval_MilliSec = 100;
	mew_mc20.EnableLoc = 0;
}
...
{  
  ...
  do
  {
    ...
    mew_mc20_SendBuff_Hook(pReq.pBuff, pReq.Length);    
  ...
  //while(1)
  {
    ...
    mew_mc20_SendBuff_Hook(pReq.pBuff, pReq.Length);		
		...
		{			
			...
			if(fi2 > 0)
			{
				memset(mew_mc20.IMEI, 0, 20);
				memcpy(mew_mc20.IMEI, pRecv.pBuff + pResp.Length, fi2 - pResp.Length);
				*(recvStream.pLength) = 0;
				return 0;
			}
		}
		...
		{			
			...
			if(fi2 > 0)
			{
				...
				for(fi = 0; fi < fi2 - pResp.Length; fi ++)
				{
					if(databuff[fi] == ',')
					{
						memcpy(mew_mc20.Longitude, databuff, fi);
						memcpy(mew_mc20.Latitude, databuff + fi + 1, fi2 - pResp.Length - fi);
					}
				}
				*(recvStream.pLength) = 0;
				return 0;
...
static int8_t AT_QIOPEN(uint8_t ch, char *pIP, uint16_t pPORT, uint32_t sec)
{
	...
	
  {
    ...
    mew_mc20_SendBuff_Hook(pReq.pBuff, pReq.Length);
		...
		mew_mc20_DelayMS_Hook(AT_CMD_TIMEOUT_MSEC);
    ...
    mew_mc20_SendBuff_Hook(pReq.pBuff, pReq.Length);
		...
		else
		{
			...
			else
			{
				...
				if(fi >= 0)
				{
					mew_mc20_SocketRecvStart_Hook(ch);
					for(i = 0; i < 4; i ++)
					{
					...
					{
						payload.pBuff = mew_mc20_Malloc_Hook(payload.Length);
						if(payload.pBuff)
						{
							memcpy(payload.pBuff, fi + pRecv.pBuff + pResp.Length + i + 2, payload.Length);
							mew_mc20_SocketRecvDone_Hook(ch, payload.pBuff, payload.Length);
							mew_mc20_Free_Hook(payload.pBuff);
							*(recvStream.pLength) = 0;
							return payload.Length;
						}
						else
						{
							mew_mc20_MallocErr_Hook();
						}
					}
					else
					{
						mew_mc20_SocketRecvParseErr_Hook(ch);
					}
				}	
				else
				{
...
{
	int8_t ret = AT_QIOPEN(ch, mew_mc20.IP[ch], mew_mc20.PORT[ch], 10);
...
{	
	int16_t ret = 0;
	isRecv = 1;
	ret = AT_QIRD(ch, mew_mc20.IP[ch], mew_mc20.PORT[ch], 256, 1000);
	isRecv = 0;
	if(ret >= 0)
	{
...
static void socket_Schedule_NoOS(void)
{
	...
	if(mew_mc20.WorkState == MC20_WS_RESETING)
	{		
		mew_mc20_ResetStart_Hook();
		if(0 <= reset())
		{
			mew_mc20.WorkState = MC20_WS_RESET;
			mew_mc20_Reset_Hook();
//			if(mew_mc20.EnableLoc)
			{
//				GPSEnable();
			}
		}
	}		
	if(mew_mc20.WorkState == MC20_WS_GPRS_CONNECTING)
	{
		if(0 <= GPRSConn())
		{			
			mew_mc20_GPRSConnDone_Hook();
			mew_mc20.WorkState = MC20_WS_GPRS_CONNECTED;
		}
		else
		{
			mew_mc20_GPRSConnErr_Hook();
			mew_mc20.WorkState = MC20_WS_RESETING;
		}
	}
	if(mew_mc20.WorkState == MC20_WS_TCP_CONNECTING)
	{		
		if(0 < AT_QIDNSGIP(mew_mc20.ADDR[socketConnectingChannel], mew_mc20.IP[socketConnectingChannel], DNSGIP_TIMEOUT_SEC))
		{
			mew_mc20_SocketConnErr_Hook(socketConnectingChannel, -2);
			//return 0;
		}
		...
		{
			mew_mc20_SocketConnErr_Hook(socketConnectingChannel, -3);
			//close fail
		}	
		...
		if(res>= 0)
		{
			...
			mew_mc20_SocketConnDone_Hook(socketConnectingChannel);
			//gprs conn ok
		}
		else
		{
			mew_mc20_SocketConnErr_Hook(socketConnectingChannel, -1);
		}
		mew_mc20.WorkState = MC20_WS_IDLE;
	}
	
	if(mew_mc20.WorkState == MC20_WS_RESET)
	{
		mew_mc20.WorkState = MC20_WS_GPRS_CONNECTING;
	}	
	if(mew_mc20.WorkState == MC20_WS_GPRS_CONNECTED)
	{		
		mew_mc20.WorkState = MC20_WS_IDLE;
	}
	if(mew_mc20.WorkState == MC20_WS_TCP_CONNECTED)
	{
		mew_mc20.WorkState = MC20_WS_IDLE;
	}
	if(mew_mc20.WorkState == MC20_WS_IDLE)
	{
		for(i = 0; i < CHANNEL_COUNT; i ++)
		{
			if(socketEnableGet(i))//通道激活
			{
				if(socketStateGet(i))//通道已连接
				{
					...
					{
						mew_mc20_SocketSendStart_Hook(i);
						if(0 > socketSend(i, sendStream.pBuff, sendStream.Length))
						{
							socketStateClr(i);
							mew_mc20_SocketSendErr_Hook(i);
							mew_mc20_SocketDisconn_Hook(i, -2);
						}
						else
						{
							mew_mc20_SocketSendDone_Hook(i);
							sendStream.Length = 0;
						}
					}
					else//没有数据需要发送
					{
						//长时间没有数据交互，发送心跳包
						if(*systicks - lastCommTime[i] >= 1000 * mew_mc20.HeartbeatInterval_Sec)
						{
							mew_mc20_SocketHeartbeat_Hook(i);							
						}
						else//空闲
						{						
							//读数据
							if(*systicks - lastReadTime[i] >= mew_mc20.ReadInterval_MilliSec)
							{
								if(0 > socketRecv(i))
								{
									socketStateClr(i);
									mew_mc20_SocketRecvErr_Hook(i);
									mew_mc20_SocketDisconn_Hook(i, -1);
								}
							}
							else
							{
				...
				{
					if(*systicks - lastConnOpTime >= 1000 * mew_mc20.ConnectionInterval_Sec)
					{
						if(*systicks - lastConnTime[i] >= 1000 * mew_mc20.ConnectionInterval_Sec)
						{
							//socketStateClr(i);
							socketConnectingChannel = i;
							mew_mc20.WorkState = MC20_WS_TCP_CONNECTING;
